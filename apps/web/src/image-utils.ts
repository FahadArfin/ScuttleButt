...
export const MAX_BANNER_DATA_URL_LENGTH = 2_000_000;
...
/** Resize wide server banners to a crisp, shareable image below the 2 MB data URL limit. */
export async function optimizeBanner(file: File): Promise<string> {
  const image = await loadImage(file);
  ...
  for (let attempt = 0; attempt < 8; attempt += 1) {
    ...
    for (const quality of [0.86, 0.74, 0.62, 0.5, 0.38]) {
      const result = canvas.toDataURL('image/jpeg', quality);
      if (result.length <= MAX_BANNER_DATA_URL_LENGTH) return result;
    }
    scale *= 0.72;
  }
  throw new Error('That banner could not be resized below 2 MB.');
}
